"""
Phase 9: train an LSTM on sequences of engineered features to predict
binary direction, with early stopping, and compare against the
Milestone 1 baseline results (Random Forest: 59.1% accuracy, 0.631 ROC-AUC).
"""
...
def main() -> None:
    ...
    model = StockLSTM(n_features=len(feature_cols), hidden_size=16, num_layers=1, dropout=0.4)
    ...
    torch.save(model.state_dict(), MODELS_DIR / "lstm_direction_model.pt")
    print(f"Saved best model to {MODELS_DIR / 'lstm_direction_model.pt'}")
    ...
    print("\nCompare to Milestone 1 baselines (validation set):")
